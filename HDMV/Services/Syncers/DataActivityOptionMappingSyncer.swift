...
final class DataActivityOptionMappingSyncer: BaseSyncer<DataActivityOptionMapping, DataActivityOptionMappingDTO, DataActivityOptionMappingPayload> {
    ...
    override func resolveRelationships() throws {
        ...
        for mapping in allMappings {
            // Bind Activity
            if mapping.activity?.rid != mapping.activityRid {
                mapping.activity = activityCache[mapping.activityRid]
            }
            
            // Bind Option
            if mapping.option?.slug != mapping.optionSlug {
                mapping.option = optionCache[mapping.optionSlug]
            }
        }
    }
}
